...
当前开发阶段：快速分拣主流程已可用，目录导出仍在开发中（候选 `v0.5.0`）。
...
- 当前支持图片目标检测、多边形分割数据准备、图片分类式标签整理，以及客户图片的 OK/NG 快速分拣；下一步补齐目录导出和异常检测数据准备工作流。
...
- 当前已完成本地任务中心、revision/快照、保存视图、跨 split 精确重复强化和快速分拣主流程；下一步优先实现普通目录导出，再验证异常检测训练模板。近重复与预测回流按真实使用反馈进入后续阶段。详见 [当前 TODO](TODO.md) 和 [分拣与导出设计](docs/quick-sorting-directory-export.md)。
...
当前已提供大图查看与缩放、按配置生成的 1–4 键判定、详细分拣、保存后下一张、空格跳过、会话内单步撤销、位置恢复、相邻图预取和条件更新冲突保护。“分拣层级”面板统一控制操作按钮、详细字段、筛选项、统计卡片和目录结构预览。配置变化保留历史判定并标记待复核；旧策略缺少新字段时沿用原有“细分 OK＋类别和程度”行为。缺陷类型与程度默认不继承上一张。分拣不改变几何标注进度、审核状态或 split。批量预览、旧目录/标签映射和保存视图扩展仍在 TODO 中。

### 待开发的目录导出

- 已建立统一目录分桶契约：当前层级配置会写入 manifest，每张图片包含 `triage.export_bucket`，统计接口返回 `by_export_bucket`；未分级旧 OK 使用 `ok/ungraded`，缺少类型/程度分别使用 `unknown`/`ungraded`。
- 待开发的导出任务将直接使用这份分桶契约生成真实目录或 ZIP，不再让导出向导另存一套相互冲突的层级规则。
- 多缺陷图片默认只导出一份，按显式主缺陷归类；没有类型为 unknown、没有程度为 ungraded。待确认/未分拣默认排除，可选择单独导出。
- 提供写入后端所在机器的新目录和 ZIP 下载；有预检、目录/容量预览、进度、取消、重试、冻结计划与源 hash 校验。
- 导出明确复制原图，不移动、不覆盖、不合并已有目录，附 manifest.jsonl、配置和报告。远程浏览器不会把服务器路径误当作本机路径。
- 后续异常检测模板默认仅完全 OK 进入正常训练；勉强 OK 单独处理，按工件/采集组划分，NG 不进入纯正常训练。具体算法版本的数据加载验证通过后才声明可直接使用。

### 开发顺序

1. 完成 Q1/Q2 剩余项：批量预览、快照分拣差异、旧目录/标签映射和压力/窄屏验收。
2. Q3：普通目录与 ZIP 导出、冻结计划、清单和安全发布；形成第一轮可试用闭环。
3. Q4：异常检测训练模板、分组划分和实际数据加载验证。
4. Q5：真实用户对照试用，修复阻断并根据复用意愿决定后续能力。
...
- 规划中的“分拣目录导出”将显式复制图片到新的目录或 ZIP，并附完整清单；它与现有标签导出是不同入口，当前版本尚不提供。
