...
当前已发布版本：`v0.4.1`；当前开发候选：`v0.5.0`

当前开发阶段：快速分拣与普通目录/ZIP 导出已形成可试用闭环（候选 `v0.5.0`）。
...
当前已提供大图查看与缩放、按配置生成的 1–4 键判定、详细分拣、保存后下一张、空格跳过、会话内单步撤销、位置恢复、相邻图预取和条件更新冲突保护。“分拣层级”面板统一控制操作按钮、详细字段、筛选项、统计卡片和目录结构预览。配置变化保留历史判定并标记待复核；旧策略缺少新字段时沿用原有“细分 OK＋类别和程度”行为。缺陷类型与程度默认不继承上一张。分拣不改变几何标注进度、审核状态或 split。批量预览、快照分拣差异和保存视图扩展仍在 TODO 中。
...
1. 完成 Q1/Q2 剩余项：批量预览、快照分拣差异、旧普通标签映射评估和压力/窄屏验收。
...
构建与验收说明见 [packaging/windows/README.md](packaging/windows/README.md)。发布 ZIP 为 `dist/DatasetManager-windows-x64-portable.zip`。
