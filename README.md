...
| OK 等级 | 完全 OK、勉强 OK | 勉强 OK 表示按当前标准仍合格 |
| 缺陷类型 | 两级类型、可多选、可指定主缺陷 | 可以只判定 NG，不立即细分类型 |
| 缺陷程度 | 轻微、中等、严重、未评估 | 可只选程度，也可与类型同时记录 |
| 判定依据 | 数据集标准与版本、分拣备注 | 区分客户标准，改变标准后提示复查 |

例如“可接受浅划痕”可记录为勉强 OK＋划痕＋轻微；“关键区小裂纹”可记录为 NG＋裂纹＋轻微。程度不会自动决定合格与否。完全 OK 与有效缺陷字段冲突时需要用户明确处理。

当前已提供大图查看与缩放、1–4 键判定、详细分拣、保存后下一张、空格跳过、会话内单步撤销、位置恢复、相邻图预取和条件更新冲突保护。缺陷类型与程度默认不继承上一张。分拣不改变几何标注进度、审核状态或 split。批量预览、旧目录/标签映射和保存视图扩展仍在 TODO 中。

### 待开发的目录导出

- 普通模板：按判定、按缺陷类型、按严重程度、类型＋程度。默认区分 `ok/clear` 与 `ok/borderline`，NG 可细分 defect 或程度目录。
