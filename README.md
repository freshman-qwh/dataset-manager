...
下一阶段：快速分拣与目录导出（候选 `v0.5.0`，设计已整理，功能尚未实现）。
...
- 当前支持图片目标检测、多边形分割数据准备与图片分类式标签整理；下一阶段围绕客户数据的快速分拣、目录导出和异常检测数据准备补齐工作流。
...
- 当前已完成本地任务中心、revision/快照、保存视图和跨 split 精确重复强化；下一步优先实现“快速分拣 → 普通目录导出”，再验证异常检测训练模板。近重复与预测回流按真实使用反馈进入后续阶段。详见 [当前 TODO](TODO.md) 和 [分拣与导出设计](docs/quick-sorting-directory-export.md)。
...
计划提供大图/原图查看、键盘判定、详细分拣、保存后下一张、跳过、会话内单步撤销、断点恢复和有限批量预览。缺陷类型与程度默认不继承上一张。分拣不改变几何标注进度、审核状态或 split。
...
1. Q1：结构化分拣字段、缺陷类型、标准与版本、条件更新、查询和快照兼容。
2. Q2：快速分拣页、连续队列、恢复/撤销、批量操作和精细标注衔接。
3. Q3：普通目录与 ZIP 导出、冻结计划、清单和安全发布；形成第一轮可试用闭环。
4. Q4：异常检测训练模板、分组划分和实际数据加载验证。
5. Q5：真实用户对照试用，修复阻断并根据复用意愿决定后续能力。
